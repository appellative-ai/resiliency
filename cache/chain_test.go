...
func ExampleChain() {
	...
	agent.hostName = "www.google.com"

	url := "https://localhost:8080/search?q=golang"
	...
	fmt.Printf("test: CacheAgent [status:%v ] [encoding:%v] [cached:%v]\n", r.Result().StatusCode, r.Result().Header.Get(iox.ContentEncoding), r.Result().Header.Get("cached"))
